...
export function What(field) {
  return tDoc.getField(field)
    ? tDoc.getField(field).value
    : BackwardsCompatible[field]
    ? eval(BackwardsCompatible[field])
    : "";
}

export function Who(field) {
  return tDoc.getField(field) ? tDoc.getField(field).userName : "";
}

export function How(field) {
...
export function AddTooltip(field, tooltip, submitNm) {
  if (!tDoc.getField(field)) return false;
  if (tooltip !== undefined) tDoc.getField(field).userName = tooltip;
  if (submitNm !== undefined) tDoc.getField(field).submitName = submitNm;
}

export function SwapTooltip(field1, field2) {
  tt1 = Who(field1);
  tt2 = Who(field2);
...
export function Checkbox(field, FldValue, tooltip, submitNm) {
  if (!tDoc.getField(field)) return false;
  var Checkit = FldValue === undefined ? true : FldValue;
  var checkNo = isArray(tDoc.getField(field).page)
    ? tDoc.getField(field).page.length
    : 1;
...
export function PickDropdown(field, FldValue) {
...
export function format1(extraDec, fixedDec, unit) {
...
export function keystroke1(allowDec, allowNegative) {
  if (!event.willCommit) {
    ...
    if (allowDec) {
      var tests =
        ...
        (/,|\./g.test(event.change) &&
          (!/,|\./g.test(event.value) ||
            /,|\./g.test(event.value.substring(event.selStart, event.selEnd))));
    } else {
...
export function FormatDay() {
  ...
  event.value =
    event.value && isDate
      ? util.printd(What("DateFormat_Remember"), isDate)
      : "";
}

//make sure the date is entered in the correct format (keystroke)
export function KeystrokeDay() {
...
export function RoundTo(inputNmbr, roundNmbr, emptyAtZero, applyDec) {
  var input = isNaN(inputNmbr)
      ? Number(inputNmbr.replace(/,/g, "."))
      : inputNmbr,
    result = inputNmbr;
  ...
  if (roundNmbr && !isNaN(roundNmbr)) {
    if (roundNmbr >= 1) {
      ...
      result =
        Math.round(input * Math.pow(roundNmbr, -1)) / Math.pow(roundNmbr, -1);
    }
  }
...
export function NormDecimal(dec) {
  var i = 0;
  var first = ding.match(/,|\./);
  var result = dec.replace(/,|\./g, function (all, match) {
    return i++ === 0 ? first : "";
  });
  return result;
}
...
export function ObjLength(theObj) {
  var size = 0;
...
export function thermoM(input, remove) {
  if (input === "start" || !IsNotImport || IsNotImport === "no progress bar")
    return "";
  var t = app.thermometer;
  if (!input || input.toLowerCase() == "stop") {
    if (!thermoStopSet && t.text != undefined)
      thermoStopSet = app.setTimeOut("thermoStop();", 500);
    return "";
  }
  var dT = 10;
  if (remove !== undefined && isNaN(input)) {
    // remove the input if remove = true, or the latest entry if remove = false
  ...
  if (!remove && isNaN(input)) {
    // start new with the input text
    if (t.text == undefined) {
      t.begin();
      t.duration = dT;
    }
  ...
  } else if (!remove && t.text != undefined) {
    // update progress with the input number (if there is an active progress bar)
    t.value = dT * input;
...
export function testFont(fontTest) {
  var remFont = tDoc.getField(
    (tDoc.info.AdvLogOnly ? "AdvLog." : "") + "Player Name"
  ).textFont;
  try {
    tDoc.getField(
      (tDoc.info.AdvLogOnly ? "AdvLog." : "") + "Player Name"
...
export function clean(input, remove, diacretics) {
  if (remove && isArray(remove))
    remove = remove.join("").replace(/(-|\\|\^|\])/g, "\\$1");
  var removeRegex = remove
    ? RegExp("/^[" + remove + "]+|[" + remove + "]+$", "g")
    : /^[ \-.,\\/:;]+|[ \-.,\\/:;]+$/g;
...
export function MakeRegex(inputString, extraRegex) {
  return RegExp(
    "^(?=.*\\b" +
      inputString
        .replace(/\W/g, " ")
...
export function toSup(inString) {
...
export function setDialogName(dialogElem, itemID, attrNm, setAttr) {
  if (dialogElem.description && dialogElem.description.elements)
    dialogElem = dialogElem.description.elements;
...
export function RollD(die) {
  return Math.floor(Math.random() * die) + 1;
}

//set the other checkbox Dis/Adv off when clicking this field (on MouseUp)
export function SetDisAdv() {
  var Adv = /Adv$/.test(event.target.name);
  this.getField(
    event.target.name.replace(Adv ? "Adv" : "Dis", Adv ? "Dis" : "Adv")
  ).value = "Off";
}

//see if two strings don't differ too much in length
export function similarLen(str1, str2) {
  return (
    Math.abs(str1.length - str2.length) < 5 ||
...
export function isTemplVis(tempNm, returnPrefix) {
  ...
  if (!multiTemp) {
    ...
    isVisible =
      (isArray(tempPage) ? Math.max.apply(Math, tempPage) : tempPage) !== -1;
  } else {
  ...
  if (isVisible && prefixes) {
    ...
    for (var i = 1; i < prefixes.length; i++) {
      if (!tDoc.getField(prefixes[i] + bkmrkFld))
        errorPrefixes.push(prefixes[i]);
    }
  ...
  return isVisible && returnPrefix && firstTempl
    ? [isVisible, returnPrefix === "last" ? lastTempl : firstTempl]
    : isVisible;
}

// Error fixing: remove pages without form fields
export function pageErrorCleanup(tempNm, prefixes) {
  ...
  for (var i = 0; i < tDoc.numPages; i++) {
    if (!pagesWithFields[i]) {
      ...
      var goOn =
        expectedPageNo.indexOf(i) !== -1 ||
        expectedPageNo.indexOf(i + 1) !== -1 ||
        expectedPageNo.indexOf(i - 1) !== -1; // Test if matches (or close enough) an expected page number
      if (!goOn) {
        // Didn't match, so ask the user if this page should be deleted
        var userResponse = app.alert({
          cMsg: "Delete erroneous page " + (i + 1) + "?",
          cTitle:
            "Page number " +
            (i + 1) +
            " seems to have no fields on it. This is probably due to an error when generating new pages or deleting them." +
...
export function eval_ish(inStr) {
  return /function|=>/.test(inStr)
    ? eval(inStr)
    : Function("return " + inStr)();
}

// Returns the template prefix, or true if not the template, or an empty string if rEmpty is true
export function getTemplPre(tName, templ, rEmpty) {
  templ = templ + ".";
  return tName.indexOf(templ) === -1
    ? rEmpty
      ? ""
      : true
    : tName.substring(0, tName.indexOf(templ)) + templ;
}

// Create the semantic version of the input (x.y.z-preRelease+build)
export function getSemVers(version, preRelease, build) {
  var strV = version
    .toString()
    .replace(/^\D+/, "")
    .split(/[\.,\-\+;]+/); // remove prefix that is not a number and split by dividers
...
/*
	...
	11.95b				-> 11095000.5
*/
export function semVersToNmbr(inSemV) {
  ...
  var arrV = [strV.join(""), parseFloat(strV[0]) < 13 ? "000" : "900"];
...
export function calcStop(noSheetUpdate) {
  noSheetUpdate =
    noSheetUpdate !== undefined ? noSheetUpdate : !IsNotReset || !IsNotImport;
  app.calculate = false;
  tDoc.calculate = false;
  tDoc.delay = true;
  if (!calcStartSet)
    calcStartSet = app.setTimeOut(
      "calcCont(" + noSheetUpdate + ", true);",
      250
    );
}

// function to start the calculations of the PDF again
export function calcCont(noSheetUpdate, viaTimeOut) {
...
export function sourceDate(srcArr) {
  if (!srcArr) return 0;
  srcArr = !isArray(srcArr) ? [srcArr] : [].concat.apply([], srcArr);
  var dateArr = [0];
  for (var i = 0; i < srcArr.length; i++) {
    var src = srcArr[i];
    if (!SourceList[src] || CurrentSources.globalExcl.indexOf(src) !== -1)
      continue;
    var srcDate =
      src === "SRD"
        ? 1
        ...
        ? 90001231
        : SourceList[src].date
        ? Number(SourceList[src].date.replace(/\D/g, ""))
        : "stop";
...
export function outOfMemoryErrorHandling(closeFile) {
  var cMsg =
    'The imported custom content JavaScript has returned an "Out of Memory" error. This means that your machine does not have enough free RAM to use this PDF without errors. This can be caused by other software using too much RAM, but most of the time is the result of the operating system not being restarted for too long.';
  cMsg +=
    closeFile || !tDoc.dirty
...
export function getLetterRange(str, aOpt) {
