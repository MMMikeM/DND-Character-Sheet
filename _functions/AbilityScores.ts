...
export function processStats(
  ...
  var alsoHasMax =
    inAlsoHasMax && isArray(inAlsoHasMax) ? [].concat(inAlsoHasMax) : false;
  ...
  for (var s = 0; s < scoresA.length; s++) {
    if (
      ...
      if (iCurScore + scoresA[s] < alsoHasMax[s]) {
        alsoHasMax[s] =
          iCurScore + scoresA[s] > 20 ? iCurScore + scoresA[s] : 0;
      ...
      maxIsLimitToNow &&
      alsoHasMax &&
      CurrentStats.maximumsLimited &&
      CurrentStats.maximumsLimited[NameEntity]
    ) {
    ...
    if (AddRemove && !dialogTxt && s < 7) {
      var theScoreName =
        s < 6 ? AbilityScores.names[s] : What("HoSRememberState");
      var theScore =
        type == "override"
          ? theScoreName + " is " + scoresA[s]
          : type == "maximum"
          ? theScoreName + " maximum is " + scoresA[s]
    ...
    if (isSpecial) {
      ...
      if (aMods.length)
        curStat.scores[s] = processModifiers(curStat.scores[s], aMods);
    } else {
      if (AddRemove) {
  ...
  if (
    alsoHasMax &&
    alsoHasMax.reduce(function (acc, val) {
      return acc + val;
    }, 0) === 0
  )
    alsoHasMax = false;
  ...
  if (AddRemove) {
    ...
    if (
      !CurrentStats.txts[inType][NameEntity] ||
      CurrentStats.txts[inType][NameEntity].indexOf(useDialogTxt) === -1
    ) {
      CurrentStats.txts[inType][NameEntity] =
        (!dialogTxt && CurrentStats.txts[inType][NameEntity]
          ? CurrentStats.txts[inType][NameEntity] + "; "
          : "") + useDialogTxt;
    } else if (
      !dialogTxt &&
    ...
    if (
      type === "background" &&
      !ObjLength(CurrentStats.txts.background) &&
      Number(curStat.scores.join("")) === 0
    ) {
...
function AbilityScores_Button(onlySetTooltip) {
  ...
  for (var aClass in classes.known) {
    ...
    var primeAbi =
      multiClass && tClass.prereqs ? tClass.prereqs : tClass.primaryAbility;
    if (primeAbi) primeAbi = primeAbi.replace(/^( |\n)*.*: |;$/g, "");
    refTxt.push(clHead + primeAbi);
    // String for ASI from class level
    var imprLVL = Math.min(
      classes.known[aClass].level,
      tClass.improvements.length
    );
    if (tClass.improvements[imprLVL - 1])
      asiTxt.push(clHead + "\xD7" + tClass.improvements[imprLVL - 1]);
  }
  if (refTxt.length) {
    refTxt.sort();
  ...
  if (asiTxt.length) {
    asiTxt.sort();
    sections.asi.txt =
      "Add 2 points to ability scores -or- take 1 feat:\n" +
      asiTxt.join(";\n") +
      ".";
  }
  ...
  for (var sType in CurrentStats.txts) {
    ...
    for (var sName in CurrentStats.txts[sType]) {
      tArr.push(
        "\u2022 " + toUni(sName) + ": " + CurrentStats.txts[sType][sName]
      );
    }
    if (tArr.length) {
      tArr.sort();
  ...
  var openStatsDialog = function () {
        ...
        elements: [
          {
            ...
            char_width:
              theStat.type == "items"
                ? 4
                : theStat.name.toLowerCase().indexOf("override") == -1
                ? 5
                : 6,
            height: 30,
    ...
    var AbilityScores_Dialog = {
      ...
      initialize: function (dialog) {
        ...
        for (var i = 0; i < CurrentStats.cols.length; i++) {
          ...
          for (var s = 0; s < 7; s++) {
            if (thisCol.scores[s])
              toSet[aNo + asab2[s]] = thisCol.scores[s].toString();
          }
        }
      ...
      updateTotal: function (dialog, fldNm) {
        ...
        if (!fromItems) {
          ...
          toSet =
            theOverride >= theTotal ? theOverride : Math.min(theMax, theTotal);
        } else {
          ...
          for (var aMax in CurrentStats.maximums[indx]) {
            ...
            if (isNaN(thisMax.substring(0, 1)) && !isNaN(thisMax.substring(1)))
              thisMax = theMax;
          ...
          var overRef = Math.min(
            theOverride + itemRef,
            Math.max(alsoAddTheMax ? theMax : 20, theOverride)
          );
      ...
      cAdB: function (dialog) {
        // add a column
        this.setCurrentStats(dialog);
        dialog.end("cadd");
      },

      cReB: function (dialog) {
        // remove a column
        this.setCurrentStats(dialog);
        dialog.end("crem");
      },
    ...
    var addFldFunction = function (i, fldID) {
      var a = fldID;
      AbilityScores_Dialog[a] = function (dialog) {
        this.updateVals(dialog, a, a.indexOf("00") === 0);
      };
    };
    for (var i = 0; i < CurrentStats.cols.length; i++) {
      var theStat = CurrentStats.cols[i];
...
function ASaddColumn(inputName, isBackground) {
  ...
  var diaText =
    "The field is intentionally small so that you have an idea of how big the caption can be. If something doesn't fit nicely, it will definitely not display correctly in the ability score dialog.\n\nIf you include the word 'override' in the caption, the column will be treated as an overriding column instead of an adding column. This means that a value will be used if higher than the other values added together.";
  var diaText2 =
    "If you leave the above field blank, no column will be created.";
  var theDialog = {
    initialize: function (dialog) {
      dialog.load({
        img1: allIcons.scores,
  ...
  var newColName = inputName
    ? inputName
    : app.execDialog(theDialog) === "ok" && theDialog.column
    ? theDialog.column
    : false;
  if (newColName) {
...
function ASremoveColumn() {
  ...
  var diaText =
    "Removing a column can't be undone once you press 'Apply' in the ability scores dialog! Any values in the column will then forever be lost.";
