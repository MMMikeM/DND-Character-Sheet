type Race = {
  ...
  sortname?: string;
  spellcastingAbility?: any;
  spellcastingBonus?: any;
  speed: Speed;
  toolProfs?: (string | [string, number])[];
  trait: string;
  variants?: string[];
  vision?: (string | [string, number])[];
...
var Base_RaceList: Record<string, Race> = {
  dragonborn: {
      ...
      "draconic ancestry": {
        ...
        calcChanges: {
          atkAdd: [
            function (fields, v) {
              if (
                v.theWea.dbBreathWeapon &&
                CurrentRace.known === "dragonborn"
              ) {
                fields.Damage_Die =
                  (CurrentRace.level < 6
                    ? 2
                    : CurrentRace.level < 11
                    ? 3
                    : CurrentRace.level < 16
                    ? 4
                    : 5) + "d6";
                if (CurrentRace.variant) {
                  ...
                  fields.Description = fields.Description.replace(
                    /(dex|con) save/i,
                    (/cold|poison/i.test(CurrentRace.dmgres[0])
                      ? "Con"
                      : "Dex") + " save"
                  );
                  fields.Range = /black|blue|brass|bronze|copper/i.test(
                    CurrentRace.variant
                  )
                    ? "5-ft \xD7 30-ft line"
                    : "15-ft cone";
  ...
  "half-elf": {
    regExpSearch:
      /^(?=.*half)(?=.*(elf|elv|drow|silvanesti|qualinesti|grugach|kagonesti)).*$/i,
    name: "Half-elf",
    source: [
      ["SRD", 6],
      ["P", 39],
  ...
  "half-orc": {
    ...
    heightMetric:
      " range from 1,5 to well over 1,8 metres tall (150 + 5d10 cm)",
  ...
  tiefling: {
    regExpSearch:
      /^((?=.*tiefling)|(?=.*planetouched)(?=.*(hell|abyss|fiend|devil|asmodeus))).*$/i,
    name: "Tiefling",
    source: [
      ["SRD", 7],
      ["P", 43],
    ...
    features: {
      "hellish rebuke": {
        ...
        spellChanges: {
          "hellish rebuke": {
            description:
              "Cast when taking damage, creature that dealt damage takes 3d10 Fire damage; save halves",
            changes:
              "Using Infernal Legacy, I cast Hellish Rebuke as if I'm using a 2nd-level spell slot, doing 3d10 damage.",
          },
        },
      },
      darkness: {
